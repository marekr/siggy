...
export class Astrolabe extends Activity {
	...
	*/
		$('#astrolabe-new-waypoint-form').submit( function(e)
		{
			...
			StaticData.systemTypeAhead.search(systemName, sync, false);
		});
...
*/
	...
	private getRouteRules(secFilter)
	{
		if( secFilter == 'safest' )
		{
			return {
	...
	private getRequestStruct(from, to)
	{
		var base = {
			"method": "Route.Find",
			"params": [{
	...
	private needsCacheUpdate(cached, waypoint)
	{
	...
	private populateRouteTable(table, path)
	{
		table.empty();
		...
		for(var i in path)
		{
			...
			var system = StaticData.getSystemByID(step.solarSystemID);
