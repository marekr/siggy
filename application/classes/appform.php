  ...
  /**
	...
	public function password($name, $value = NULL, array $attributes = NULL) {
     ...
     // $result = '<li>'.Kohana_Form::password($name, $value, $attributes);
      $result = Kohana_Form::password($name, $value, $attributes);
      // add error span
      if (isset($this->errors[$name])) {
         $result .= '<span class="error">'.ucfirst($this->errors[$name]).'</span>';
      } else if (isset($attributes['info'])) {
	...
	public function checkbox($name, $value = NULL, $checked = FALSE, array $attributes = NULL) {
      $this->load_values($name, $value, $attributes);
      $result = Kohana_Form::checkbox($name, $value, $checked, $attributes);
      // add error span
      if (isset($this->errors[$name])) {
         $result .= '<span class="'.$this->error_class.'">'.ucfirst($this->errors[$name]).'</span>';
      } else if (isset($attributes['info'])) {
	...
	public function radio($name, $value = NULL, $checked = FALSE, array $attributes = NULL) {
      $this->load_values($name, $value, $attributes);
      $result = Kohana_Form::radio($name, $value, $checked, $attributes);
      // add error span
      if (isset($this->errors[$name])) {
         $result .= '<span class="'.$this->error_class.'">'.ucfirst($this->errors[$name]).'</span>';
      } else if (isset($attributes['info'])) {
	...
	public function yes_no($name, $value = NULL, $attributes = NULL) {
      $this->load_values($name, $value, $attributes);
      $result = "<label class='yes'>Yes ";
			$result .= Kohana_Form::radio($name, 1, ($value == 1 ? TRUE : FALSE), $attributes);	
      $result .= "</label><label class='no'>No ";
			$result .= Kohana_Form::radio($name, 0, ($value == 0 ? TRUE : FALSE), $attributes);	
      $result .= "</label>";
      return $result;
	}
	...
	public function textarea($name, $body = '', array $attributes = NULL, $double_encode = TRUE) {
      ...
      $result = Kohana_Form::textarea($name, $body, $attributes, $double_encode);
      // add error span
      if (isset($this->errors[$name])) {
         $result .= '<span class="'.$this->error_class.'">'.ucfirst($this->errors[$name]).'</span>';
      } else if (isset($attributes['info'])) {
