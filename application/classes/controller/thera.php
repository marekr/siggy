...
class Controller_Thera extends FrontController
{
	...
	*/
	...
	public function action_latest_exits()
	{
		...
		$cache_name = 'thera-exits-evescout';;

		if( ($data = $cache->get( $cache_name, FALSE )) == FALSE )
		{
			...
			if( $data != null )
			{
				$cache->set($cache_name, $data, 300);
			}
		}
		
		
		$wormholes = DB::query(Database::SELECT, 'SELECT id, name
													FROM statics')
		...
		foreach( $data as $rawExit )
		{
			...
			$system = DB::query(Database::SELECT, 'SELECT s.id, s.name, s.sysClass as sys_class, s.region as region_id, r.regionName as region_name, s.sec
														FROM solarsystems  s
