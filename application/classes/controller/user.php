...
class Controller_User extends Controller_App {
   ...
   public function action_profile() {
      ...
      if ( simpleauth::instance()->logged_in() == false ){
      ...
      $view->set('user', simpleauth::instance()->get_user() );
   }
 ...
 //     $view->set('user_roles', $roles);
      ...
      if ($_POST) {
         ...
         try {
           ...
           $userData = array('username' => $_POST['username'],
														 ...
														 'email' => $_POST['email'],
														 'active' => 1 );
						if( simpleauth::instance()->create_user( $userData, TRUE ) )
						{
            // sign the user in
            ...
            {
               Message::add('error', __('An unknown error has occured.'));
               die();
            }
					//	die();
         } catch (ORM_Validation_Exception $e) {
   ...
   public function action_login() {
      ...
      if($this->request->is_ajax() && isset($_REQUEST['username'], $_REQUEST['password'])) {
         ...
         // If user already signed-in
         if(simpleauth::instance()->logged_in() != 0){
            // redirect to the user account
            $this->request->redirect('user/profile');
         }
         ...
         if ($_REQUEST && isset($_REQUEST['username'], $_REQUEST['password'])) {
            ...
            if ( simpleauth::instance()->login($_REQUEST['username'], $_REQUEST['password']) ) {
               // redirect to the user account
               $this->request->redirect('user/profile');
               return;
            } else {
         ...
         if(isset($_GET['username'])) {
            $view->set('username', HTML::chars($_GET['username']));
         }
   ...
   public function action_forgot() {
      ...
      if(isset($_POST['reset_email'])) {
         //$user = ORM::factory('auth_user')->where('email', '=', $_POST['reset_email'])->find();
  ...
  function action_reset() {
      ...
      if(isset($_REQUEST['reset_token']) && isset($_REQUEST['reset_email'])) 
      {
         ...
         if( (strlen($_REQUEST['reset_token']) == 32) && (strlen(trim($_REQUEST['reset_email'])) > 1) ) 
         {
						...
						{
               Message::add('failure', __('You have attempted to use an invalid reset token.'));
						}
            else if ($user->admin == 1) 
            {
               Message::add('failure', __('An admin password cannot be reset by email.'));
            } 
