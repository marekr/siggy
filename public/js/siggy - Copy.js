...
siggymain.prototype.update = function ()
{
	...
	$.get(this.baseUrl + 'update', request, function (data)
	{
		...
		{
			that.acsname = data.acsname;
			$('#updateTime span.acsname').text(that.acsname);
		}
...
siggymain.prototype.addSigRow = function (sigData, fromUpdateCycle)
{
	var that = this;
	...
	.append($('<td>').addClass('center').addClass('type').text(this.convertType(sigData.type)))
	.append(descTD)
...
siggymain.prototype.editSigForm = function (sigID)
{
	...
	sigEle.append( $('<input>').val(this.sigData[sigID].sig).addClass('sigEdit').attr('maxlength', 3).keypress( function(e) { if(e.which == 13){ that.editSig(sigID)  } } ) );
	...
	descEle.text('');
	descEle.append(this.generateSiteSelect(this.systemClass, this.sigData[sigID].type, this.sigData[sigID].siteID)).append($('<br />')).append($('<input>').val(this.sigData[sigID].description).keypress( function(e) { if(e.which == 13){ that.editSig(sigID)  } } ));
...
siggymain.prototype.registerMenu = function(ele, dir, optionCallbacks)
{
		$('#'+ele).data('opened', false);
		$('#'+ele).data('disabled', false);
		
		$('#'+ele).click(function(event) { //When trigger is clicked...  
				...
				menu.show(); //Drop down the subnav on click  
				
				if( dir == 'up' )
				{
					var offset = -1*(menu.height())-5;
		...
		for( var i in optionCallbacks )
		{
			if( typeof optionCallbacks[i] == 'function' )
			{
				$('#'+i).click( function(event)
				{
					...
					optionCallbacks[ $(this).attr('id') ].call(this);
